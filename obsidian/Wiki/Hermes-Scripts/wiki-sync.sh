...
# ------------------------------------------------------------------------------
# Git Repo는 상위 폴더에 있음
REPO_DIR="/home/comage/obsidian-vault"
WIKI_SUBDIR="06-Wiki-시스템"
WIKI_DIR="$REPO_DIR/$WIKI_SUBDIR/Wiki"
RAW_DIR="$REPO_DIR/$WIKI_SUBDIR/Raw"
LOGS_DIR="$REPO_DIR/$WIKI_SUBDIR/Logs"
...
process_wiki_docs() {
    ...
    for file in $(git diff --name-only "HEAD..origin/$BRANCH" 2>/dev/null); do
        # Wiki 폴더 내부 파일만 (06-Wiki-시스템/ 접두사 포함)
        if [[ "$file" != "$WIKI_SUBDIR/"* ]]; then
            continue
        fi
        
        # 06-Wiki-시스템/ 접두사 제거
        relative_path="${file#$WIKI_SUBDIR/}"
...
process_deleted_file() {
    ...
    if [[ "$remote_file" == *"$WIKI_SUBDIR/Wiki/"* ]]; then
