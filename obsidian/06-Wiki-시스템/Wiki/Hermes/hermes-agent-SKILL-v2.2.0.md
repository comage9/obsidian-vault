...
metadata:
  hermes:
    ...
    related_skills: [claude-code, codex, opencode, hermes-docs-reference]
---
...
Hermes provides **two approaches** for web access:

### Approach 1: Browser Automation (CDP)

Full browser environment — use when you need to interact with dynamic pages, fill forms, click buttons, or handle JavaScript-rendered content.

| Tool | Purpose |
|------|---------|
| `browser_navigate(url)` | Load a page and get a compact snapshot of interactive elements |
| `browser_snapshot(full=false)` | Refresh the snapshot after interactions; `full=true` for complete text |
| `browser_click(ref)` | Click an element by its ref ID (e.g. `@e5`) |
| `browser_type(ref, text)` | Type into an input field |
| `browser_scroll(direction)` | Scroll up/down to reveal more content |
| `browser_vision(question)` | Screenshot + visual analysis; **use for CAPTCHAs, visual layout, image content** |
| `browser_console(expression)` | Run JavaScript in page context (DOM inspection, data extraction); also reads console errors |
| `browser_get_images()` | List all images on current page with URLs and alt text |
| `browser_back()` | Navigate back in history |
| `browser_press(key)` | Keyboard key press (Enter, Escape, Tab, ArrowDown) |

**Best for:**
- Dynamic web apps (React, SPA)
- Sites requiring login/authentication
- CAPTCHA verification pages
- Visual inspection of page layout
- Forms that need clicking/typing

**Requirements:** Local Chromium, Browserbase, or Camofox backend.

### Approach 2: Direct HTTP (curl / API)

Faster, lower-overhead — use for plain-text endpoints, APIs, markdown files, JSON endpoints.

| Tool | Use case |
|------|----------|
| `terminal(command="curl ...")` | API calls, file downloads, raw HTTP |
| `terminal(command="python3 ...")` | Scripted API access with response parsing |

**Best for:**
- REST APIs returning JSON
- Raw text/markdown files (.md, .txt, .json, .yaml)
- GitHub raw content (raw.githubusercontent.com)
- Any documented API endpoint

**Rule of thumb:** Prefer `terminal` + `curl` for plain data; use `browser_navigate` only when you need to interact with a rendered page.

### Toolset configuration

```bash
hermes tools enable web       # web_search + web_extract
hermes tools enable browser   # browser_navigate + browser_vision + ...
hermes tools enable search    # web_search only (lightweight)
```

Toolsets are per-platform — `hermes tools` toggles them interactively for each connected platform. Changes take effect on `/reset`.
...
Features not detailed elsewhere in this skill:

| Feature | Description |
|---------|-------------|
| **Browser Automation** | `browser_navigate`, `browser_click`, `browser_type`, `browser_snapshot`, `browser_vision()`, `browser_console()` — CDP-based Chromium automation |
| **Code Execution** | `execute_code()` — sandboxed Python with hermes_tools import, 5-min timeout, 50KB stdout cap |
| **Computer Use** | Desktop GUI control (experimental); macOS variant via bundled `macos-computer-use` skill |
| **Web Search** | `web_search()`, `web_extract()` tools for web content retrieval |
| **Context Files** | AGENTS.md, CLAUDE.md, .cursorrules auto-loaded when `workdir` is set (cron) or detected at startup |
| **Plugins** | `hermes plugins list/install/remove` — types: context-engine, memory-provider, image-gen-provider, model-provider |
| **LSP** | Language Server Protocol integration for code-aware tooling |
| **Image Gen** | `image_gen` toolset — ComfyUI integration via bundled skill |
| **Vision** | `vision_analyze()` — image analysis with vision-capable models or fallback |
| **TTS** | `/voice tts` — Edge(무료), ElevenLabs, OpenAI, MiniMax, Mistral, NeuTTS |
| **Spotify** | `spotify` toolset — play/search/queue/manage playlists |
| **Goals** | `/goal [text]` — standing goal across turns; subcommands: status/pause/resume/clear |
| **Hooks** | Event hook system for shell integrations (`~/.hermes/shell-hooks-allowlist.json`) |
| **Personality** | `/personality [name]` — switch persona mid-session |
| **Skins** | `/skin [name]` — TUI theme switching |
| **Tool Gateway** | External tool gateway for web search / image gen / TTS / browser |
| **Provider Routing** | Route specific tasks to specific providers |
| **Fallback Providers** | Automatic failover when primary provider errors |
| **Checkpoints** | `/rollback [N]` — filesystem checkpoint/restore (config: `checkpoints.*`) |
| **Subscription Proxy** | Gateway subscription proxy for multi-platform routing |
| **X Search** | X/Twitter search integration |
| **Batch Processing** | Batch task processing |
| **Deliverable Mode** | Artifact/deliverable output mode |

### Learning Path

| Level | Focus |
|-------|-------|
| **Beginner** | Installation → Quickstart → Configuration → Skills → Memory |
| **Intermediate** | Cron → Delegation → MCP → Profiles → Gateway |
| **Advanced** | Developer Guide → Plugin System → Adding Tools/Providers |

---

## Messaging Platforms Reference

All supported gateway platforms with setup notes:

| Platform | Config section | Notes |
|----------|---------------|-------|
| **Telegram** | `telegram.bot_token` | Button UI, `/model` command, topic sessions via `/topic` |
| **Discord** | `discord.bot_token` | Slash commands; **Message Content Intent** required in Bot → Privileged Gateway Intents |
| **Slack** | `slack.bot_token` | **`message.channels`** event subscription required for public channels |
| **WhatsApp** | `whatsapp.*` | Meta Business API; phone number ID, access token |
| **Signal** | `signal.*` | Requires signal-cli daemon (`signal-cli daemon`) |
| **Email** | IMAP/SMTP | himalaya CLI; SMTP relay support |
| **Matrix** | `matrix.*` | `require_mention` config; homeserver URL + access token |
| **Mattermost** | `mattermost.*` | Server URL + bot token |
| **DingTalk** | `dingtalk.*` | Robot webhook + OAuth |
| **Feishu** | `feishu.*` | App ID + App Secret |
| **WeCom** | `wecom.*` | Corp ID + Agent ID + Secret |
| **WeChat** | `weixin.*` | WeChat (Weixin) Official Account |
| **QQ Bot** | `qqbot.*` | QQ bot platform integration |
| **SMS** | Twilio | Twilio account SID + auth token + phone number |
| **ntfy** | `ntfy.*` | Push notification topic; simple HTTP POST |
| **LINE** | `line.*` | LINE Messaging API channel |
| **Simplex** | `simplex.*` | Simplex chat platform |
| **Google Chat** | `google_chat.*` | Google Workspace Chat API |
| **Home Assistant** | `homeassistant.*` | Smart home platform + `homeassistant` toolset |
| **Open WebUI** | API Server | Connects via Hermes API Server adapter |
| **BlueBubbles** | iMessage | BlueBubbles server for iMessage on macOS |
| **Teams** | Microsoft Graph | Teams meeting pipeline (bundled skill); Microsoft Graph API |
| **Webhooks** | HTTP | Custom webhook routes at `/webhooks/<name>` |
| **Yuanbao** | `yuanbao.*` | Yuanbao (元宝) platform integration |

### Common Gateway Config
- `channel_prompts` — per-channel prompt overrides
- `require_mention` — whether bot must be mentioned
- `auto_thread` — Discord auto-thread creation
- `history_backfill` — backfill message history count
- `cooldown` — per-channel rate limiting
- `max_message_length` — platform message size limit
...
| Guide | Description |
|-------|-------------|
| **Work with Skills** | Creating, managing, and browsing skills |
| **Automate with Cron** | Scheduling recurring tasks |
| **Cron Script-Only** | `no_agent=True` watchdog scripts |
| **Cron Troubleshooting** | Debugging cron failures |
| **Delegation Patterns** | Subagent spawn strategies |
| **Use MCP with Hermes** | Connecting MCP servers |
| **Use Voice Mode** | STT/TTS configuration |
| **Daily Briefing Bot** | Cron-based daily summary bot |
| **GitHub PR Review Agent** | Automated PR review via cron + delegation |
| **Team Telegram Assistant** | Multi-user Telegram assistant |
| **Local LLM on Mac** | Ollama + Hermes on macOS |
| **Local Ollama Setup** | Detailed Ollama configuration |
| **Python Library** | Using Hermes as a Python library |
| **Tips & Best Practices** | Prompting, tool use, context management |
| **Use SOUL with Hermes** | SOUL.md persona files |

Official guides: https://hermes-agent.nousresearch.com/docs/guides/
...
|---------|--------|---------|
| **Secret Redaction** | `security.redact_secrets` | On (snapshotted at import — needs restart to change) |
| **PII Redaction** | `privacy.redact_pii` | Off |
| **Command Approval** | `approvals.mode` | `manual` (prompt before destructive commands) |
| **Website Blocklist** | `security.website_blocklist` | Empty |
| **Tirith Policy Engine** | `security.tirith_enabled` | Off (experimental) |
| **YOLO Mode** | `--yolo` flag or `approvals.mode: off` | Off |

**Key distinction:** Secret redaction (API keys in tool output) ≠ PII redaction (user IDs in gateway messages). They are independent.
...
| Date | Change |
|:-----|:-------|
| 2026-06-07 | v2.1.0 → v2.2.0 — Merged `hermes-docs-reference` content + added Web Search & Browser Tools dedicated section |
| Original | v2.1.0 — Bundled skill as shipped with Hermes Agent |
