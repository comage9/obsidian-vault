...
### 제품배치도 — 크로스동 이동 + 임시 보관함 (백업 backup-20260817-pre-cross-dong)
- **총괄 탭 크로스동 이동**: 미니맵에서 A동 제품→C동 등 다른 동으로 드래그 (기존 zone 브랜치 재사용, 소스 제거+대상 append). A동 점유 칸 append 가드(1칸 1품목). 수정 모드에선 미니맵 드래그 비활성.
