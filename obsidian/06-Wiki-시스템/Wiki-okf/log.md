...
## 2026-08-18
- [정리] 제품배치도 기능 확장 + Claude Code 위임 체계 (2026-08-18) — 리뷰 반영 3건(H1/H2/H3) + 슬림 분리 rank-a-v20 + 동별 출고 비율 + 동별 TOP 품목(Claude Code 위임). 문서: 의사결정/제품배치도-동별출고비율-탑품목-슬림분리-20260818.md
- [수정] 제품배치도 동별 TOP 품목 표시 (2026-08-18) — 동별 출고 비율 카드 각 동 컬럼에 상위 3개 품목(번호·이름·박스) 표시. Claude Code 위임 구현 (2.1.234, max-turns 40). 실측: A동=153번 663박스. Git 5b6bced
- [수정] 제품배치도 동별 출고 비율 (2026-08-18) — 우측 상단 '📦 동별 출고 비율 (최근 1개월)': 동별 배치 제품 1개월 출고 합계 → % + 막대바 + 총박스. 실측 A77.4/B7.3/C11.3/D4.1%. Git 24b1f0b
- [수정] 제품배치도 A동 슬림 분리 재배치 (2026-08-18) — 슬림서랍장(10)→L2-18~19+L3-1~8, 슬림형 서랍장(5)→L3-9~13 전용 블록. L4=로코스14+이유5, L5=L5=이유/북/옷걸이, L6=옷걸이/초대형/맥스/탑백, L7=와이드/트레이/데크/해피/소분류. 탑백 222/223·슬림 244/214/241 침범 정리. zoneOrderA L7형식 지원. rank-a-v20 (localStorage 리셋, 2번 L2-1 복귀). Git 714b487
- [수정] 제품배치도 리뷰 반영 (2026-08-18) — Claude Code 리뷰 3건: H1(zid 앞 중복 시 입력 덮어쓰기 방지), H2(다품목 칸 rest 보존), H3(수정OFF input 잔류). ref 가드 + staging 함수형 통합. Git bf8a9c7
- [수정] 제품배치도 수정 모드 가드 (2026-08-18) — 일반 모드에서 칸 클릭 시 편집 차단, 수정 모드에서만 인라인 입력/다이얼로그 허용. Git 2d859f5
- [수정] 제품배치도 중복 배치 시프트 (2026-08-18) — 19 위치에 33 추가 시 33이 있던 칸 제거 + 이후 한 칸씩 당겨 맨 끝 빈칸. applyPlacementEdit 공통 헬퍼(인라인/다이얼로그 모두 적용). Git 3b369bb
- [수정] 제품배치도 저장 확실성 강화 (2026-08-18) — 인라인 blur 자동 저장 + 저장 성공 메시지(✅ 칸 저장). Git 55b5663
- [수정] 제품배치도 배정 다이얼로그 직접 입력창 추가 (2026-08-18) — 검색창에만 입력하면 저장 안 되던 버그. '직접 입력(콤마 구분)'창 추가, confirmAssign=임시보관함 연동 통일. Git 688249a
- [수정] 제품배치도 칸 직접 입력(수기) 모드 (2026-08-18) — 드래그앤드롭 대체. 칸 클릭→인라인 input, 콤마 구분 다품목(19,28), Enter 저장/Esc 취소, 빠진 제품→📦임시보관함 자동 이동, 임시보관함 제품 칸 입력 시 자동 제거, A동 다품목 표시. Git 13d76d3
- [수정] KPP 인쇄 오늘 날짜 전표 강제 (2026-08-18) — 출차관리 KPP 버튼이 이전 날짜 등록분 인쇄하는 버그 수정: ①행0 폴백 제거 ②col8(출하일) 날짜 필터 ③PDF URL dlv_dat 검증. kpp_session.py + KPP MCP server.py. Git 83f7829
- [수정] A동 지그재그 L1↔L2 페어 배치 (1-1=1·2-1=2·1-2=3·2-2=4...) + 분류 라인 블록(모던 L1~L2, 슬림+로코스 L3~L4, 나머지 L5~L7·X) + 단수 순 (Git 2d38b01)
- [수정] 제품배치도 배치 내역 개수 unique 품목 수로 통일 (572→515) — 같은 pnum 다중 칸 배치(42종)로 위치 수 vs 품목 수 불일치 (Git 4f9eedc)
- [문서] `물류/VF/제품배치도-A동-지그재그-단수순-20260818.md` — A동 지그재그 배치(L1~L6 순환, 1-1=1·2-1=2) + 단수 순(모던 5→6→4→3→2, 슬림 8→7→6→5→4→3→2), 배치 탭 개수 뱃지, 동별 패널 따라다님, 툴팁 1개월 출고 N박스/W원 (Git 4bd0fc5·2f6a5a1)
- [문서] `물류/VF/제품배치도-A동-132칸-상위132-20260818.md` — A동 132칸 전체 배치(1개월 출고 상위 132품목, 칵투스 11 제외·임시보관함, 모던38→슬림14→로코스19→나머지61), 989 슈즈랙 X1 배치 (Git f4689f1)
- [문서] `물류/VF/제품배치도-출고순-스왑-20260818.md` — A동 출고 순 스왑 9건(배치 하위↔미배치 상위, 같은 분류 내), 989 슈즈랙 1개월 출고 0박스 표기 버그 수정(실시간 30일 계산, 27박스), 목록에 출고 N박스 표시 (Git c4e88c4)
...
- [정상] A동 칸 툴팁 품명 masterMap 기준 수정 (2026-08-19) — makeTooltip이 A_ZONE_*[zid] 칸 하드코딩이라 제품 이동 시 엉뚱한 품명 표시(2-16=28 배치해도 35 이름) → 배치 제품번호별 masterMap[pn] 나열로 변경 · 브라우저 실측 2-16→모던플러스 28 / 1-17→30,35 둘 다 표시 · 커밋 d063911. 이어지는 제품번호 파생규칙 교정(e6793d1)과 함께 배치도 품명 일관성 완료
- [정상] 배치 시프트 조건 + 크로스동 중복 + 총괄 검색 수정 (2026-08-19) — ①2-17에 36,37 입력 시 편집칸 앞 빈칸은 당김 생략(1-19만 빈칸, 2/601 이동없음) ②A동 배치 시 B/C/D동 동일 pnum 자동 제거(전역 1칸 유일 — 36이 A/C동 중복이던 문제 해결) ③검색 클릭 시 동 이동 대신 총괄 축소카드 flash 표시 · S1~S5 PASS + 브라우저 실측 · 커밋 6ccb667. 상세: 물류/VF/제품번호-파생규칙-교정-20260819.md 계열
- [정상] 미배치 내역 출고 상위순 정렬 (2026-08-19) — CategoryList sortBy='qty' 옵션(분류 그룹핑 없이 출고 내림차순 플랫 리스트+헤더) + unplaced boxes 실제 출고량 계산 · 미배치 탭 적용(246품목, 292=145박스 1위) · 사용자 직접 입력/지시 워크플로우 기반 · 커밋 1956a5f
- [정상] 제품배치도 배치 로직 일관성 통일 (2026-08-19) — 4개 리뷰(Claude Code+서브에이전트3) 감사로 경로 7개 제각각 문제 확인 · removeCrossDongDupes 공통 헬퍼로 전 경로 전역 1칸 유일 강제 · 드래그 push모델→pull모델 통일(수기와 동일) · 자리이탈/그룹/overflow 경로 applyPlacementEdit 또는 aSeq 물리정렬 사용 · 커밋 29693c4 · 잔여: 기존 localStorage 크로스동 중복 57건(배치 시 자동정리)
- [정상] 제품배치도 전면 개편 완료 (2026-08-19) — 위치 고정 모델 확정: 품목 드래그 시 대상 칸만 변경·다른 칸 이동 없음(Playwright 실측: 36번 1-19→1-18 드래그, 변경 칸 3개=src/dst/크로스동중복), 빈 칸은 그대로 유지. 칸 편집: 위아래=선택 칸 레이아웃 교환, 휴지통 드롭존, 빈 칸 추가=라인 끝 정렬. 임시보관함: 우측 패널+세로 확장+분류 그룹+번 표시+드래그 배치. 배치 내역: 행 드래그 배치(RowDrag)+번 접미사. tsc 0건. 커밋 29693c4+fabaf7a push 완료 — Wiki/제품배치도-전면-개편-위치-고정-20260819.md
- [정상] 제품배치도 A동 체인 밀어내기 추가 (2026-08-19) — 점유 칸에 드롭 시 기존 제품이 다음 칸으로 연쇄 이동(aChainInsert), 첫 빈 칸에서 종료, 전 칸 점유 시 마지막만 임시보관함. B/C/D→A 점유 칸 드롭도 동일 동작(차단 해제). 순수 로직 Node 실측: S1 999→1-13 드롭=200→1-14, 201→1-15(빈칸) 종료·33 무변동 PASS / S2 전칸 점유=203만 overflow PASS. 위치 고정 원칙 유지(src 당김 없음). 커밋 6843ab6 push

### 23:30 — 일일 크론 갱신 (8/19)
- [수정] 출차 페이지 LS/KPP 인쇄 파이프라인 복구 (2026-08-19 저녁) — 원인 2가지: backend `.venv`에 pywin32/websocket-client 미설치 + Django 재시작 필요. 복구 후 전 호차 인쇄 성공: 1호차 전남80바2805(PLT 12)·2호차 경기89바1454(PLT 11)·3호차 광주90바1703(PLT 14) — LS PDF + KPP EDI 전표 Canon G2010 출력, 차량번호·PLT 일치 검증. 사용자 1호차 방식 정상 확인. 함정: 시스템 python에는 있어도 .venv에 pywin32 없으면 printto 실패
- [정상] 출고데이터 동기화 `7fb0ae1fa347` 09:15 — 신규 427건(8/17 216 + 8/18 211), 갱신 0, 누적 257,617건 · Auto-Watcher 차량 3대 등록
- [정상] LS 14시 통합 `8a776545148d` — 로그인 성공(welcomePage) · 오늘(8/19) 3건 이미 등록 SUBMITTED(29540071=90269/3호차11T·29540072=90626/1호차5T·29540073=90628/2호차5T) → 배치 생성 스킵
- [실패] LS PDF 인쇄 `8c57a12b627d` 16:30 — 쿠키 파일 없음 **5일 연속(8/15~19)**. 단, 저녁에 departure 페이지 🖨️ 수동 인쇄는 정상 복구(위 항목)
...
### 07:00 — 출고 데이터 동기화 (cron `7fb0ae1fa347`)
|- `manage.py daily_outbound_sync` (Python313, `VF-new - 복사본/backend`)
|- DB 최신: 2026-08-19 → 오늘 2026-08-21
|- 결과: **신규 220건, 갱신 216건** (기준일: 2026-08-19~), exit 0
|- Auto-Watcher: 다운로드 폴더 스캔 → 신규 차량 3대 등록

### 14:11 — LS 14시 통합 (cron `8a776545148d`)
|- [차단] ls-coupang 스킬 부재 지속 — 쿠키 파일 없음 (`coupang_cookies_browser.txt`) → LS 로그인 필요 (8/13~8/21 9일 연속 동일 패턴)
|- [참고] mandatory-verification 스킬 로드 확인, ls-coupang 스킬 미발견으로 skip

## 2026-08-22

### 13:58 — 제품배치도 모던플러스·로코스 바코드 기준 배치 완료 (vf-op 세션)
|- 제품배치도(`/product-display`) 칸 표시: 제품명 → **제품번호 전용** 복원 (ZoneCell·MiniZoneCell)
|- 모던플러스 25개 로케이션 1~25 (5단→4단→6단→3단→2단), 로코스 21개 로케이션 26~46 순차 배치
|- 마스터 `product_number` 필드 오염 확인 → **바코드 → location → 제품번호 파생** 규칙 사용
|- 자가 치유 로직(`rank-a-v23`): 파일 SoT 강제 교정 + 옛 자리 14곳 삭제 + 밀려난 4개(695/692/690/2146) 임시보관함 이관
|- 브라우저 실측 검증: 모던 25/25 + 로코스 21/21 일치, tsc 신규 에러 0
|- 상세: `물류/VF/제품배치도-모던플러스-로코스-바코드배치-20260822.md` · VF 커밋 46932ef
