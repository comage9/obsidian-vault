...
**⚠️ Pitfall: "토큰 응답에 access_token 이 없음"은 키 환경변수 미설정也表示**
- `KIWOOM_APP_KEY`가 비어있으면 API 서버가 토큰을 거부 → 같은 에러
- 크론 실행 전 반드시 `echo $KIWOOM_APP_KEY`로 확인
-详见 `kiwoom-api-auth` 스킬의 references/missing-env-vars-diagnostic.md
